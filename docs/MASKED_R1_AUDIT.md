...
## Final issue tracking results

| Audited issue | R1 implementation | Actual result |
|---|---|---|
| A/M symmetry and missing target | relation_supervision.py | CPU cases 1–3 PASS |
| Unconstrained strength, conflated U/M/correctness | soft_edl_loss and Dirichlet statistics | CPU 4–6 PASS; real U calibration NOT RUN |
| P_C(1-U) mislabeled belief | conflict_belief=b_C | CPU identities PASS |
| Unsigned deletion, disabled probe supervision | reference_subset_probe.py, evidence_utility.py | CPU 7–11 and real tiny reference pipeline PASS; real-data target quality NOT RUN |
| Named minority head without contribution target | one signed utility head + offline minority report | Synthetic execution PASS; useful minority mechanism NOT SUPPORTED yet |
| Pair semantics/missing-token leakage | availability_mask and run_layers | CPU eager/SDPA/BF16 Jacobian and nonconstant endpoint checks PASS; CUDA NOT RUN |
| Multiple compensating scores and dual fusion | single masked-softmax and weighted sum | CPU 16–20 PASS; baseline performance NOT RUN |
| IURD detach/residual deletion problems | no IURD instantiated in R1 | Parameter and gradient checks PASS |
| Threshold/checkpoint/budget confounds | factory, train/evaluate/engine, cache identity | CPU real CLI/Gloo/checkpoint tests PASS; real matched studies NOT RUN |
| E/X derived views mistaken for causes | explicit raw intervention vs latent deletion | API/CPU test PASS; no causal claim |

Snapshot SHA256: `8b944acc3371bed44f8fa60cf44ef20c14fadb79ae0547111d95d7e964a16b27`.
